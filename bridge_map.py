...
def log_p_beta_eval(beta, lambda_obj, alpha, phi):
  px = beta.shape[0]
  ans = px*( torch.log(alpha) + ( torch.log(phi) + torch.log(lambda_obj) )/alpha - torch.lgamma(1/alpha) ) 
  ans = ans - torch.squeeze( lambda_obj*phi*torch.sum( torch.pow(torch.abs( beta ), alpha ) ) )
  return ans
...
def log_p_alpha_eval(alpha):
  ...
  p_alpha_05 = torch.exp( p_alpha_dist_05.log_prob(alpha/2) ) / 2
  p_alpha_1 = torch.exp( p_alpha_dist_1.log_prob(alpha/2) ) / 2
  p_alpha_2 = torch.exp( p_alpha_dist_2.log_prob(alpha/2) ) / 2
...
#-------------------------------------------------------------------------------
  ...
  while keep_going:
    ...
    # log-likelihood
    log_lik_train = log_lik_eval(y = y_minib, X = X_minib, Z = Z_minib, beta = beta_var, gamma = gamma_var, phi = phi_var)
    ...
    if epoch % 50 == 0:
      ...
      if minibatch == minibatches_per_epoch - 1:   
        ...
        # Save info on gamma
        list_gamma[ :, index_save] = gamma_var.detach().numpy()
        ...
        print(f'Epoch: {epoch},   Minibatch: {minibatch},   Training objective: { round(full_log_joint, 2) },  gamma: { round(gamma_var.item(), 2)},  phi: { round(phi_var.item(), 5)}, lambda: { round(lambda_var.item(), 2)}, alpha: { round(alpha_var.item(), 2)}' )
